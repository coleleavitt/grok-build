    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            let (response, latency) = match self.run_turn_via_sampler(request.clone()).await {
                ...
                Ok(outcome @ SamplerTurnOutcome::ProviderAccountRotated) => {
                    debug_assert!(
                        auth_retry_decision_for_outcome(&mut auth_retry_schedule, &outcome)
                            .is_none()
                    );
                    self.send_xai_notification(XaiSessionUpdate::RetryState(
                        crate::extensions::notification::RetryState::Retrying {
                            attempt: 1,
                            max_retries: 1,
                            reason: "Provider account rate-limited; rotated account and retrying request"
                                .to_string(),
                        },
                    ))
                    .await;
                        ...
                        AuthRetryDecision::UnchargedResubmit { resubmit } => {
                            ...
                            self.send_xai_notification(XaiSessionUpdate::RetryState(
                                crate::extensions::notification::RetryState::Retrying {
                                    ...
                                    reason: "Re-authenticated after 401 (request carried no credential); retrying request".to_string(),
                                },
                            ))
                            .await;
                        ...
                        | AuthRetryDecision::RunawayGuard { .. }) => {
                            ...
                            let message = match decision {
                                AuthRetryDecision::RunawayGuard { rejections } => format!(
                                    "Auth recovery kept succeeding but {rejections} requests were rejected (401) before a credential could be sent, with no successful response in between; stopping as a runaway guard.{duration_note}"
                                ),
                                _ if authenticated == rejections => format!(
                                    "Auth recovery succeeded but {rejections} authenticated inference requests were still rejected (401); giving up after {} retries.{duration_note}",
                                    AuthRetrySchedule::MAX_RETRIES
                                ),
                                _ => format!(
                                    "Auth retry budget exhausted after {rejections} post-recovery 401s ({authenticated} provably carried a credential).{duration_note}"
                                ),
                            };
                            ...
                            return Err(acp::Error::internal_error().data(
                                crate::sampling::error::error_data_with_status(message, Some(401)),
                            ));
                        }
                    }
                }
...
/// Charge the bounded post-recovery 401 budget only for outcomes that
/// represent an actual 401. Provider-account rotation follows a separate 429
/// path and must leave an open 401 incident untouched.
fn auth_retry_decision_for_outcome(
    schedule: &mut AuthRetrySchedule,
    outcome: &SamplerTurnOutcome,
) -> Option<AuthRetryDecision> {
    match outcome {
        SamplerTurnOutcome::RefreshAuthAndResubmit { credential, .. } => {
            Some(schedule.on_recovered_401(*credential))
        }
        SamplerTurnOutcome::ProviderAccountRotated
        | SamplerTurnOutcome::Response(_, _)
        | SamplerTurnOutcome::CompactAndResubmit => None,
    }
}

#[cfg(test)]
mod provider_rotation_retry_tests {
    use super::*;

    #[test]
    fn provider_rotations_never_charge_the_401_budget() {
        let mut schedule = AuthRetrySchedule::new();
        for _ in 0..5 {
            assert!(
                auth_retry_decision_for_outcome(
                    &mut schedule,
                    &SamplerTurnOutcome::ProviderAccountRotated,
                )
                .is_none()
        ...
        let recovered_401 = SamplerTurnOutcome::RefreshAuthAndResubmit {
            credential: xai_grok_sampling_types::SentCredential::Sent,
            store: RecoveredStore::SessionToken,
        };
        assert!(matches!(
            auth_retry_decision_for_outcome(&mut schedule, &recovered_401),
            Some(AuthRetryDecision::Backoff { attempt: 1, .. })
        ));
    }
}
