...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        for source in cmds {
            ...
            // `apply_classifier_outcome_legacy`, which owns its own rollback. The
    ...
    /// Apply a legacy `update_goal` classifier outcome and return the tool ack.
    ...
    /// Apply an evaluator-backed verification outcome to goal state.
    ...
    async fn account_not_achieved_without_sampler(
        ...
        let gaps_summary = reason.gaps_summary();
        {
            ...
            // `apply_classifier_outcome_legacy`'s real NotAchieved branch.
            tracker.record_not_achieved_streak();
        }
    ...
    pub(super) async fn resume_goal(&self) -> GoalResumeOutcome {
        ...
        // Make the accepted paused → active transition durable before the
        // planner await below, so a crash cannot resurrect the old pause.
        if was_resumed {
    ...
    /// Workflow-engine goal loop: evaluate the completed worker round, verify
    /// candidate completion, and decide whether to continue in-turn.
    ...
    /// Legacy `update_goal` loop: drain completion claims and continue in-turn.
