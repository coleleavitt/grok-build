...
impl SessionActor {
    ...
    pub(super) async fn run_verification_stage_for_drain(
        ...
        if let Some(outcome) = self
            .maybe_run_goal_review_before_verification(
                &objective,
    ...
    async fn maybe_run_goal_review_before_verification(
        &self,
        objective: &str,
        ...
        if !self.goal_review_enabled {
            return None;
        }
        let agent = crate::session::goal_review::SubagentReviewAgent {
            event_tx,
            parent_session_id: self.session_id_string(),
            parent_prompt_id,
            cwd: Some(self.tool_context.cwd.as_str().to_owned()),
        };
        let (report, paths) = match crate::session::goal_review::run_post_change_review(
            &agent,
            true,
            objective,
            attempt,
            crate::session::goal_review::ReviewEffort::Max,
            baseline_commit,
        ...
        .await
        {
            Ok(result) => result,
            Err(err) => {
                tracing::warn!(error = %err, "goal review: failed open before verifier");
                return None;
            }
        };

        crate::session::goal_review::completion_block_for_report(&report, &paths).map(|block| {
            crate::session::goal_classifier::GoalClassifierOutcome::NotAchieved {
                details_path: block.details_path,
                gaps_summary: block.gaps_summary,
                pause_summary: block.pause_summary,
                gap_fingerprint: block.gap_fingerprint,
            }
        })
    }
