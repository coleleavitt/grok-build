...
#[cfg(test)]
use responses::build_responses_input;
...
mod tests {
    ...
    fn test_messages_request_thinking_carries_summarized_display() {
        ...
        assert_eq!(
            json.pointer("/thinking/display")
                .and_then(|value| value.as_str()),
            Some("summarized"),
            "thinking.display must be 'summarized' so supported models surface thinking content; got: {json:#}",
        );
    }
